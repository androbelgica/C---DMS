...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
            ...
            <div class="doc-infor">
                <div class="card shadow-sm">
                    ...
                    <div class="card-body file-info-box">
                        ...
                        <div class="row mb-3">
                            <label class="card-title">File Source</label>
                            <div class="col-sm-10">
                        ...
                        <div class="row mb-3">
                            <div class="col-sm">
                                ...
                                <asp:Button ID="btnOpenCamera" runat="server" Text="Open Camera" CssClass="btn btn-sm camera-file" OnClick="btnOpenCamera_Click" Visible="false" />
    ...
    <script>
        ...
        // Function to open the camera, prioritizing back camera on iOS if available
        function openCamera() {
            // Function to try accessing the camera with a specific facing mode
            function tryAccessCamera(facingMode) {
            ...
            // Determine facing mode order based on platform
            let facingModeOrder;
            if (iOS) {
                // iOS specific facing mode order (back camera first, then front camera)
                facingModeOrder = ['environment', 'user'];
            } else {
                // Default facing mode order (front camera first)
                facingModeOrder = ['user', 'environment'];
            }

            // Try accessing the cameras in the defined order
            function tryCameras(orderIndex) {
                if (orderIndex >= facingModeOrder.length) {
                    // No suitable camera found, fall back to any available camera
                    return navigator.mediaDevices.getUserMedia({ video: true });
                }
                return tryAccessCamera(facingModeOrder[orderIndex])
                    .then(function (mediaStream) {
                        ...
                        video.play().catch(function (err) {
                            // Autoplay was prevented. Show controls and allow user to start video manually.
                            video.controls = true;
                        });
                    })
                    .catch(function (err) {
                        console.warn('Failed to access camera with facing mode ' + facingModeOrder[orderIndex], err);
                        // Try next facing mode
                        return tryCameras(orderIndex + 1);
                    });
            }

            // Start trying to access cameras
            tryCameras(0)
                .catch(function (err) {
        ...
        async function processPDF(file) {
            ...
            reader.onload = async function (e) {
                ...
                for await (const image of imageIterator) {
                    ...
                    console.log(`Processed page ${pageCounter} of ${numPages}`);
                }
        ...
        function displayPageImage(imageURL) {
            const filedisplayContainer = document.getElementById('<%= filedisplayContainer.ClientID %>');
            filedisplayContainer.innerHTML = ''; // Clear existing images
            const img = document.createElement('img');
            img.src = imageURL;
            filedisplayContainer.appendChild(img);
            filedisplayContainer.style.display = 'block';
